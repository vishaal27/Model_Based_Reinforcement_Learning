...
# Loading VAE
vae_file = join(args.logdir, 'ae', 'best.tar')
...
def to_latent(obs, next_obs):
    ...
    with torch.no_grad():
        ...
        # print(len(a[0]))
        # print(len(a), a[0][0].shape,a[0][1].shape, a[1][0].shape, a[1][1].shape)
        # va=AE(3, LSIZE).to(device)
        # z , next_z = [va.forward(x)[1:] for x in (obs, next_obs)]
        ...
        latent_obs, latent_next_obs = [
            ...
            [(obs_mu, obs_logsigma), (next_obs_mu, next_obs_logsigma)]]
    # print(latent_obs, latent_next_obs)
    return latent_obs, latent_next_obs #z[0], next_z[0]
    ...
    latent_obs, action,\
        ...
        latent_next_obs = [arr.transpose(1, 0)
                           for arr in [latent_obs, action,
                                       reward, terminal,
                                       latent_next_obs]]
    # print("action shape",action.shape," latent_obs shape ",{latent_obs.shape})
